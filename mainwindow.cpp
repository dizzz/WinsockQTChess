...
void MainWindow::newgame(bool f){
    if(hisnewgame == YES && mynewgame == YES){
        ...
        Common::turn = !Common::turn;
        init();
        Common::playing = Common::turn;
       // Common::sockthread->setReceiving(!Common::playing);
        if(!Common::playing && !Common::sockthread->isReceiving() && !f && mynewgame == NOT_SET)
             //Common::sockthread->setReceiving(1);
            Common::sockthread->sendMsg(SockData::HELLO,"");
        Game::chessBoard(this, tile,Common::turn);
        update();
        setEnabled(true);
    }else{
        Game::gameover = 1;
